...
func toRoman(num int) (string, error) {
	if num <= 0 || num > 10 {
		return "", fmt.Errorf("Римские числа допустимы только для чисел от 1 до 10")
	}

	romanNumerals := map[int]string{1: "I", 4: "IV", 5: "V", 9: "IX", 10: "X"}

	result := ""
	for value := 10; value > 0; {
		if num >= value {
			result += romanNumerals[value]
			num -= value
		} else {
			// Используем value - 1 для правильной обработки вычитания
			value--
		}
	}
	return result, nil
